...
function ProductFP({ photo, name, category, description, alignments = {} }: ProductFPProps) {
    return (
        <div className={`p-6 bg-white text-black rounded-2xl shadow-lg flex flex-col ${alignments.container || "items-start"} text-${alignments.text || "text-left"}`}>
            <img src={photo} alt={name} className={`w-80 h-40 object-cover rounded-lg mb-4 ${alignments.photo || ""}`} />
...
interface FeaturedProductsProps {
    gridCols?: number;
    alignments?: ProductFPProps["alignments"];
}

export default function FeaturedProducts({ gridCols = 3, alignments = {} }) {
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className={`grid ${gridClass} gap-6 transition-all`}>
                    {products.map((product, index) => (
                        <ProductFP key={index} {...product} alignments={alignments} />
                    ))}
                </div>
            </div>
        </section>
