...
const FAQAccordion = ({ result }: CardProps<any>) => {
  ...
  return (
    <section className="w-full cards">
      <article className="mx-auto w-full divide-y divide-skin-base/5">
        <Disclosure as="section" className="p-2" defaultOpen={false}>
          <DisclosureButton className="group flex w-full items-center justify-between">
            <h2 className="text-skin-base text-left text-lg font-medium group-hover:opacity-80">
              {question}
            </h2>
            <ChevronDownIcon
              className={`size-5 fill-skin-base/60 group-hover:fill-skin-base/50 group-data-[open]:rotate-180 `}
            />
          </DisclosureButton>
          <DisclosurePanel
            transition
            className="text-skin-muted mt-2 flex flex-col text-secondary origin-top transition duration-200 ease-out data-[closed]:-translate-y-6 data-[closed]:opacity-0"
          >
