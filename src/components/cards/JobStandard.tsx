...
const JobStandard = ({ result }: CardProps<any>) => {
  ...
  return (
    <article
      className="flex flex-col gap-4 cards text-skin-muted"
      aria-labelledby={`job-title-${result.id}`}
    >
      <header>
        <h2
          id={`job-title-${result.id}`}
          className="text-xl font-bold text-skin-base mb-2"
        >
          {name}
        </h2>
        <p className="text-skin-base flex flex-row gap-1 w-full flex-wrap-nowrap items-center">
          {datePosted && (
            <>
      ...
      <section className="flex flex-col md:flex-row justify-between flex-nowrap">
        ...
        >
          ...
          {keywords && (
            <ul
              className="hidden md:flex flex-col md:flex-row gap-1 mt-4 flex-wrap"
