...
const DayRow = (props: DayRow) => {
  ...
  return (
    <tr className={isToday ? "bg-skin-accent" : "text-skin-muted"}>
          ...
          {({ open }) => (
            <>
              ...
              <DisclosurePanel
                transition
                as="article"
                className=" text-skin-muted origin-top transition duration-200 ease-out data-[closed]:-translate-y-6 data-[closed]:opacity-0"
              >
          ...
          {({ open }) => (
            <>
              <DisclosureButton
                ...
                className="text-start flex w-full justify-between items-center"
              >
                <h3 className="pb-4 font-medium text-skin-base">{title}</h3>
                <ChevronDownIcon
                  className={`block md:hidden size-4 fill-skin-base group-hover:fill-skin-base/50 ${
                    open ? "rotate-180" : ""
                  }`}
