    ...
    <article className="flex flex-col">
      {relatedProducts && relatedProducts.length >= 1 && (
        <section className="flex flex-col ">
          <h3 className=" text-skin-base text-2xl mb-4">Related Product</h3>
          <ul role="list" className="flex flex-col list-none gap-8">
            {relatedProducts.slice(0, 3).map((product: any, index: any) => {
              return (
                <li
                  key={index}
                  className="rounded-lg space-y-0 pb-4 bg-skin-banner border group relative top-0"
                >
                  ...
                  >
                    <FaLink className="fill-skin-base text-2xl" />
                  </a>
                  <p className="pointer-events-none block truncate font-medium text-skin-base text-lg md:text-xl p-2">
                    {product.name}
                  </p>
          ...
          {relatedProfessionals.length >= 4 && (
            <a
              ...
              className="text-skin-base flex gap-2 items-center mt-4 hover:underline"
            >
              More Professionals{" "}
              <IoIosArrowRoundForward className="h-6 w-6 fill-skin-base" />
            </a>
          )}
        </section>
      )}
      {relatedProfessionals && relatedProfessionals.length >= 1 && (
        <section className="flex flex-col w-full">
          <h3 className="text-skin-base text-2xl mb-4">
          ...
          <ul role="list" className="flex flex-col list-none gap-4 m-0 mb-8">
            {relatedProfessionals
              ...
              .map((professional: any, index: any) => (
                <li
                  key={index}
                  className="rounded-lg bg-skin-banner border group relative w-full overflow-hidden box-border"
                >
                  ...
                  <p className="pointer-events-none block truncate font-medium text-skin-base text-lg md:text-xl p-4">
                    {professional.name}
                  </p>
                </li>
              ))}
      ...
      {relatedLocations && relatedLocations.length >= 1 && (
        <section className="flex flex-col">
          <h3 className="mb-4 text-skin-base text-2xl">Related Locations</h3>
          ...
          <ul role="list" className="flex flex-col list-none gap-8">
            {relatedLocations.slice(0, 3).map((location: any, index: any) => {
              return (
                <li
                  key={index}
                  className="rounded-lg space-y-4 bg-skin-banner border p-4 group relative top-0 text-skin-muted"
                >
                  <a
                    href={`/${location.slug}`}
                    className="absolute inset-0 flex items-center justify-center bg-gray-300 bg-opacity-50 opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                  ...
                  <p className="text-lg md:text-xl  pointer-events-none block truncate font-medium text-skin-base">
                    {location.name}
                  </p>
