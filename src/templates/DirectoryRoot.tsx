      ...
      {currList && (
        <PageLayout _site={document._site} templateData={{ __meta, document }}>
          <article className="relative md:mt-6 mx-auto h-[800px] flex overflow-hidden">
            <aside className="md:absolute w-full md:w-1/4 top-12 left-12 z-30 bg-skin-banner h-[700px] space-y-6 mx-auto overflow-hidden">
              <nav className="flex justify-start items-center mx-4 mt-6">
                {historyStack.length > 0 && (
                  <button onClick={handlePrev} className="text-skin-base mr-4">
                    <ArrowLeftIcon className="h-6 w-6" />
                  </button>
                )}
                <h1 className="text-3xl text-skin-base">{currTitle}</h1>
              </nav>
                ...
                <ul className="absolute flex flex-col h-full w-full overflow-y-scroll">
                  {currList.map((item, index) => (
                    <li
                      ...
                      className={`border-skin-base flex-col gap-6 border-b last:border-b-0 py-4 first:pt-0 flex ${
                    ...
                    >
                      {!item.address && (
                        <span className="flex text-skin-muted w-full justify-between items-center px-4">
                          <h2 className="text-lg">
                            {item.c_addressRegionAbbreviation || item.name}
                          </h2>
                          {item.dm_childEntityIds?.length && (
                            <span className="flex items-center justify-between">
                              <span className="w-6 h-6 text-sm rounded-full bg-skin-base text-skin-banner flex items-center justify-center">
                                {item.dm_childEntityIds?.length || 0}
                              </span>
                              <span>
                                <BsChevronRight className="ml-1 text-skin-base h-6 w-6 stroke-1" />
                              </span>
                            </span>
                          )}
                        </span>
...
const BuildAddressCard = ({ currItem }: { currItem: DirectoryChild }) => {
  ...
  return (
    <article className="flex text-skin-base flex-col gap-2 py-4  mx-8  last:border-0 border-b border-skin-base">
