...
const InsightsAndProducts: Template<TemplateRenderProps> = ({ document }) => {
  ...
  return (
    <PageLayout _site={_site} templateData={{ __meta, document }}>
      <article
        className={` ${
          meta.entityType.id === "product" ? `bg-skin-banner` : `bg-skin-accent`
        }`}
      >
        <section
          className="centered-container py-4 flex gap-8 flex-col md:flex-row justify-between text-left w-full px-4 md:px-16 lg:px-20"
          aria-labelledby="main-content-heading"
        >
          <aside
            className="text-skin-base w-full md:w-4/5 ml-0 !text-lg flex flex-col gap-4 rounded-xl"
          ...
          <footer className="self-center  w-full md:w-1/5">
            <Cta
        ...
        <section className="centered-container md:relative mx-auto">
          <Image
            image={c_image || primaryPhoto}
            className="md:max-h-[500px] w-full rounded-lg"
          />
          <section
            className={`md:rounded-t-lg md:bg-skin-banner md:relative md:-top-48 gap-2 md:gap-8 md:mx-8 justify-between m-auto text-pretty font-medium 
